...
namespace Stprm.CajaFinanciera.Data
{
	...
	public class Descuento : Record
	{
		...
		{
			return db.QueryToAdapter ("select desc_id as Id, cat_concepto as Clave, CONCAT(CAST(desc_periodo AS CHAR),'-', CAST(desc_anio AS CHAR)) as Periodo, CAST(DATE_FORMAT(desc_fecha,'%d/%m/%Y') as CHAR)as Fecha from {0}, {1} where {0}.cat_id = {1}.cat_id order by desc_id desc",
