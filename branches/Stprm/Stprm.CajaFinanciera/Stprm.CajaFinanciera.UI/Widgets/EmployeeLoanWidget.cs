...
namespace Stprm.CajaFinanciera.UI.Widgets
{
	...
	public class EmployeeLoanWidget : CustomVBox
	{
		...
		public void UpdateFromEmployee (Employee employee)
		{
			...
			//employee.Db.QueryToAdapter ("select DATE_FORMAT(pre_fecha,'%d/%m/%Y') as Fecha, pre_folio as Folio, pre_cheque as Cheque, pre_pagare as Pagare, CONCAT('$', FORMAT(pre_capital,2)) as Capital, CONCAT('$', FORMAT(pre_interes, 2)) as Intereses, CONCAT('$', FORMAT(pre_capital + pre_interes, 2)) as Total, CONCAT('$', FORMAT(pre_abono,2)) as Abono, CONCAT('$', FORMAT(pre_saldo, 2)) as Saldo, CASE pre_status WHEN 1 THEN 'RT' WHEN 2 THEN 'DC' WHEN 3 THEN 'DT' WHEN 4 THEN 'SP' WHEN 5 THEN 'PG' END as Estado from prestamos, trabajadores where prestamos.tra_id = trabajadores.tra_id and tra_ficha='{0}'order by pre_fecha asc", employee.Id).Fill (ds);
			employee.GetPrestamosInAdapter ().Fill (ds);
